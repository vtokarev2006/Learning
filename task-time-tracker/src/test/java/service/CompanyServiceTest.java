...
public class CompanyServiceTest extends AbstractServiceForTesting {
	...
	public void testFindAll() throws Exception {
		...
		if (rowCount > 0) {
			...
			Result<List<Company>> allItems = companyService
					.findAll(TEST_USERNAME);
			assertTrue(
	...
	public void testAddNew() throws Exception {
		...
		Result<Company> c2 = companyService
				.store(null, NEW_NAME, TEST_USERNAME);
	...
	public void testUpdate() throws Exception {
		...
		companyService.store(c.getId(), NEW_NAME, TEST_USERNAME);

		Result<Company> ar2 = companyService.find(c.getId(), TEST_USERNAME);

		assertTrue(ar2.getData().getName().equals(NEW_NAME));
		assertTrue(ar2.getData().getId() != null);
	...
	public void testRemove() throws Exception {
		...
		Result<Company> ar = companyService.remove(c.getId(), TEST_USERNAME);
		...
		logger.debug("\ntestRemove() - UNABLE TO DELETE TESTSPASSED\n");
		...
		ar = companyService.remove(c.getId(), TEST_USERNAME);

		// remove should have succeeded
		assertTrue(ar.isSuccess());
		ar2 = companyService.find(c.getId(), TEST_USERNAME);
