...
public class JpaTrapTest extends AbstractDaoForTesting {
	...
	public void testManyToOne() throws Exception {
		...
		assertTrue("Original company still has project in its collection!", !c.getProjects().contains(p));

		assertTrue("Newly assigned company does not have project in its collection", c2.getProjects().contains(p));
	...
	public void testFindByUsernamePassword() throws Exception {
		...
		assertTrue("Unable to find valid user with correct username/password combination", user != null);
